...
do
	return function(UI)
		...
		local function parseThumbMeta(url)
			...
			local L = tolower(url)
			-- Default to 150x150, a common UI size. This prevents the 420x420 overflow.
			local meta = { typ = "AvatarHeadShot", w = 150, h = 150 }
			...
			if string.find(L, "^rbxthumb://", 1, true) then
				...
				-- If only one dimension is given for rbxthumb, match them
				if w and not h then meta.h = w elseif h and not w then meta.w = h end
				return meta
			end

			-- Try to find type from HTTP URL
			...
			-- NEW: Enhanced size parsing for HTTP URLs
			local w_str = url:match("[?&]w=(%d+)") or url:match("[?&]width=(%d+)")
			local h_str = url:match("[?&]h=(%d+)") or url:match("[?&]height=(%d+)")
			local size_str = url:match("[?&]size=([%dxX%d]+)") -- Check for size=150x150

			local foundW, foundH

			if size_str then
				-- Handle size=150x150
				local w_s, h_s = size_str:match("^(%d+)[xX](%d+)$")
				if w_s then foundW = tonumber(w_s) end
				if h_s then foundH = tonumber(h_s) end
			end

			-- Prioritize w/width if size= wasn't found or was incomplete
			if not foundW then foundW = tonumber(w_str) end
			if not foundH then foundH = tonumber(h_str) end

			-- Apply found dimensions. If only one is found, use it for both.
			if foundW and foundH then
				meta.w = foundW
				meta.h = foundH
			elseif foundW then
				meta.w = foundW
			...
			-- If no size params (w, h, size) are found, it will keep the 150x150 default.

			return meta
		end

		local function buildRbxThumb(uid, meta)
			local w = tonumber(meta and meta.w) or 150 -- Default to 150, not 420
			local h = tonumber(meta and meta.h) or 150 -- Default to 150, not 420
		...
		local function rewriteUserIdInUrl(url, newUid)
			...
			-- List of regex patterns to try replacing.
			-- This is more robust and catches more URL formats.
			local patterns = {
				"([?&][Uu]ser[Ii]ds?=)%d+", -- ?userIds=123, &userId=123
				"([?&]id=)%d+", 			 -- ?id=123, &id=123
				"(/users/)%d+(/)", 		 -- /users/123/
				"(/user/)%d+(/)" 			 -- /user/123/ (legacy)
			}
			...
			return nil -- No replaceable ID found in any known format
		end
		...
		local function imageShowsUserId(image, userId)
			...
			elseif image:find("roblox%.com") then
				-- Broaden check for HTTP URLs
				return image:find("userId=" .. uid, 1, true) or image:find("userIds=" .. uid, 1, true) or image:find("/users/" .. uid .. "/") or false
			end
			return false
		end
			...
			local ok, err = pcall(function()
				if avatar.EnabledCopy and avatar.CopyUserId then
					...
					if swapped then
						...
						local meta = parseThumbMeta(src) -- This is now fixed
						if meta then
							img.Image = buildRbxThumb(avatar.CopyUserId, meta) -- This is also fixed
							img.ImageTransparency = 0
						else
							-- Deep Fallback (should be rare)
							...
							local w = tonumber(src and (src:match("[?&]w=(%d+)") or src:match("[?&]width=(%d+)"))) or 150 -- Default 150
